...
def get_video_anno(video_infos,
    ...
    path_ClassIndex_Detection="STADe-DeepSeg/thumos_annotations/Class Index_Detection.txt"
    absolute_path_ClassIndex_Detection = os.path.join(absolute_path, path_ClassIndex_Detection)
    
    originidx_to_idx, idx_to_class = get_class_index_map(absolute_path_ClassIndex_Detection)
    video_annos = {}
    for anno in df_anno:
        video_name = anno[0]
        originidx = anno[2]
...
def load_video_data(video_infos, npy_data_path):
    ...
    for video_name in tqdm.tqdm(list(video_infos.keys()), ncols=0):
        ...
        data_pad[:T,:R,:C]=data
        # data = torch.from_numpy(data).view(340, 25, 30)
        # data = torch.from_numpy(data).view(340, 25, 30).unsqueeze(0)

        # data = np.reshape(data, (len(data), -1,30), order='c')
        # print(data.shape) # (8000, 9, 30)
        data = np.reshape(data_pad, (len(data_pad), -1), order='c') # [8000, 270]
        # print(data.shape) # (8000, 9, 30)
        # data = data.numpy()
        # data = np.expand_dims(data, 0).repeat(1, axis=0)
        # print(data.shape) #(1, 8000, 9, 30)
...
class THUMOS_Dataset(Dataset):
    ...
    def augment_(self, input, annos, th):
        ...
        if gt[1] < start_idx:
            ...
            # print('bad')
            # print("new_input[:, t - th:t, ]",new_input[:, t - th:t, ])
            # print("input[:, start_idx:end_idx, ]",input[:, start_idx:end_idx, ])
            # new_input[:, start_idx:t - th] = input[:, end_idx:t, ]
            new_input[:, start_idx:t - th] = new_input[:, start_idx:t - th] # 该处在程序中用不到，所以做简化处理
            new_input[:, t - th:t, ] = new_input[:, t - th:t, ]
    ...
    def __getitem__(self, idx):
        ...
        # print("video_name",sample_info['video_name'])
        # print('video_data.shape',video_data.shape) # (1, 340, 25, 30)
        ...
        # print('input_data.shape',input_data.shape) # (8000, 270)
        t, c = input_data.shape
        if t < self.clip_length:
            # padding t to clip_length
            pad_t = self.clip_length - t
            zero_clip = np.zeros([pad_t,c ], input_data.dtype)
